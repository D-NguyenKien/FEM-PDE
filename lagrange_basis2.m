function [Nv,dNdxi,dN2dxi]=lagrange_basis2(type,coord,dim)
...
%
%   * type-order: type is the toplogical class of finite element it is in the general
...
%   is B27 etc.
%   Presently defined are L2, L2H (2-node coincide with Hermite polynomial) L3,
%   T3, T4(cubic bubble), T6, Q4, Q9, H4, H10, B8 and B27 
%  
%   * coord: coord is the parent coordinates at which the basis and its
%   gradients are to be evaluated at. 
%
%   *dim: If dim is set to 2 then the vector representation of the N
%   matrix is returned.
%
%=========================================================
...
%========
% modified by D. Nguyen Kien (or Dave D.K. Nguyen)
...
%========
  ...
  switch type
   ...
   case 'L2H'
    %%%%%%%%%%%%%%%%%%%%% L2 TWO NODE LINE ELEMENT-HERMITE polynomial %%%%%%%%%%%%%%% 
    %  
    %    1-1bar---------2-2bar
    %
    if size(coord,2) <1
        ...
        xi = coord(1);
        N = ( [2-3*xi+xi^3; 1-xi-xi^2+xi^3; 2+3*xi-xi^3; -1-xi+xi^2+xi^3] / 4 );
                      %4 element, N1, N1bar, N2, N2bar
                      % or             N1, N2, N3, N4
        dNdxi = ( [-3+3*xi^2; -1-2*xi+3*xi^2; 3-3*xi^2; -1+2*xi+3*xi^2 ] /4);
        dN2dxi = ( [6*xi; -2+6*xi; -6*xi; 2+6*xi ] /4);
